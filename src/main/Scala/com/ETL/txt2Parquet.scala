...
import com.ETL.util.{SchemaUtils, Utils2Type}
...
object txt2Parquet {
  def main(args: Array[String]): Unit = {
    ...
    //按要求切割 并且保证数据长度大于等于85个字段
    //如果切割时候遇到相同切割条件重复的情况下，需要切割的话 那么后面需要加上相应匹配参数
    //这样切割才会准确 比如，，，，，会当成一个字符切割 需要加上对应的匹配参数
    val rowRDD: RDD[Row] = lines.map(x => x.split(",", x.length)).filter(_.length >= 85)
      .map(arr => {
        Row(
          arr(0),
